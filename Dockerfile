...
RUN sed -i \
  ...
  -e "s|Powered by #{link_to \"Postal\", \"https://postalserver.io\", target: '_blank'} #{Postal.version}.|Powered by #{link_to \"Postal\", \"https://postalserver.io\", target: '_blank'} #{Postal.version}.patch-1|g" \
  /opt/postal/app/lib/postal/message_db/delivery.rb \
  /opt/postal/app/lib/postal/smtp_server/client.rb \
  /opt/postal/app/views/layouts/application.html.haml
